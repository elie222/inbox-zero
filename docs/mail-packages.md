...
are the mailbox engine used by web and desktop. `@inboxzero/mail-expo` is the
Expo adapter used by the mobile app. `@inboxzero/mail-ui` is the React DOM
presentation and stays in this app.
...
`packages/mail-react`, or `packages/mail-expo` publishes those packages to npm.
All four share one version. The workflow builds each package and publishes its
`dist` manifest, in that order, and skips a version that is already on npm.
...
1. `pnpm -F @inboxzero/mail-core -F @inboxzero/mail-sqlite -F @inboxzero/mail-react -F @inboxzero/mail-expo run build`
