...
Use the Inbox Zero API to read inbox statistics, manage automation rules, and unsubscribe from senders programmatically.
...
| `SENDERS_WRITE` | `POST /senders/unsubscribe` |
...
```bash
curl -X POST "https://www.getinboxzero.com/api/v1/senders/unsubscribe" \
