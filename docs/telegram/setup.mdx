...
TELEGRAM_BOT_PROFILE_PHOTO_URL= # optional https URL to a PNG/JPG
```
...
`TELEGRAM_BOT_PROFILE_PHOTO_URL` is optional. When set, Inbox Zero checks whether the bot already has a profile photo and uploads this image only if no photo exists yet.

## 4. Telegram bot commands and profile metadata

When Telegram is configured, Inbox Zero automatically registers bot slash commands through `setMyCommands`:
...
Prompt-style commands (`/cleanup`, `/summary`, `/draftreply`, `/followups`) are mapped to common inbox-assistant prompts.

If you prefer to manage bot metadata manually (for example, replacing an existing profile photo), you can still use [@BotFather](https://t.me/BotFather).
