...
If you ran the setup CLI, start Inbox Zero with the exact Compose command setup printed. It passes your env file with `--env-file`.

To run Compose yourself from a cloned repo, pass your env file explicitly:

```bash
NEXT_PUBLIC_BASE_URL=https://yourdomain.com docker compose --env-file apps/web/.env --profile all up -d
```

Compose treats the app env file as optional so it can start on a fresh clone. If Compose can't find your file, the containers start without your settings and Compose shows no error. Passing `--env-file` avoids this.
...
For named CLI configurations, use the exact Compose command printed by setup. The generated `INBOX_ZERO_ENV_FILE` setting selects the same named file for container settings. Compose only reads `INBOX_ZERO_ENV_FILE` from the shell, from the file passed with `--env-file`, or from a repo-root `.env`, not from inside the app env file on its own. Running plain `docker compose up` with a named config ignores that file and falls back to the default env file. Existing named files created by older CLI versions also need this setting (for example, `INBOX_ZERO_ENV_FILE=./apps/web/.env.staging` in a cloned repo, or `./.env.staging` for a standalone install). Named configs use the same Compose project and do not create isolated deployments.
