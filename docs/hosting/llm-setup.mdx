...
Self-hosted deployments can choose how LLM requests handle narrow sensitive-content matches before they are sent to an AI provider. The current scanner targets likely credentials/tokens and payment-card-like numbers; it is not a full DLP or PHI classifier.

```env
AI_SENSITIVE_CONTENT_POLICY_DEFAULT=ALLOW # ALLOW, REDACT, or BLOCK
AI_SENSITIVE_CONTENT_POLICY_LOCKED=false
```

`ALLOW` preserves the default behavior. `REDACT` replaces matched values before the LLM request. `BLOCK` stops the request when a match is found. Leave `AI_SENSITIVE_CONTENT_POLICY_LOCKED=false` to let users choose per account in Settings, or set it to `true` to enforce the deployment default for all accounts.
