...
| [Cerebras](https://inference-docs.cerebras.ai/quickstart) | `cerebras` |
...
```env
DEFAULT_LLMS=openai:gpt-5.6-luna
ECONOMY_LLMS=openai:gpt-5.6-luna
LLM_API_KEY=sk-...
```
...
```env
DEFAULT_LLMS=openrouter:anthropic/claude-sonnet-5,openai:gpt-5.6-luna
ECONOMY_LLMS=openrouter:openai/gpt-5.6-luna,google:gemini-3.1-flash-lite
CHAT_LLMS=openrouter:anthropic/claude-haiku-4.5,openai:gpt-5.6-luna
```
...
```bash
cd apps/web
pnpm add ai-sdk-provider-claude-code@4.3.1
claude auth login
```
