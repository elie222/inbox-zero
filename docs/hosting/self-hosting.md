...
| **Follow-up reminders** | Every 5 minutes | `/api/follow-up-reminders` | Processes follow-up reminder labels for users with the feature enabled |
...
# Follow-up reminders - every 5 minutes (optional, only if using follow-up reminders feature)
*/5 * * * * curl -s -X GET "https://yourdomain.com/api/follow-up-reminders" -H "Authorization: Bearer YOUR_CRON_SECRET"
```
