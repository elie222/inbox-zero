...
- **Containers start but ignore your settings**: Compose couldn't find your env file and started without it. This usually happens with a named config (for example `.env.production`) or a file outside the default location, started with plain `docker compose up`
  - Solution: use the Compose command printed by `inbox-zero setup`, or pass the file explicitly: `docker compose --env-file <path-to-your-env-file> --profile all up -d`. Setting `INBOX_ZERO_ENV_FILE` inside the env file only takes effect when that file is passed with `--env-file`
