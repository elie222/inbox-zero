...
**Error: P1000 / "authentication failed" after re-running setup**
- This happens when you re-run `inbox-zero setup` which generates a new database password, but the old Postgres volume still has the previous password.
- Fix: stop containers and remove volumes so Postgres is recreated with the new password:
  ```bash
