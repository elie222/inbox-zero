...
- `LOGIN_PROVIDERS` must include `sso` (e.g. `LOGIN_PROVIDERS=sso` for an SSO-only deployment, or `LOGIN_PROVIDERS=google,microsoft,sso` to keep social sign-in alongside SSO). If `LOGIN_PROVIDERS` is unset, SSO is hidden unless the legacy `SSO_LOGIN_ENABLED=true` flag is set.
