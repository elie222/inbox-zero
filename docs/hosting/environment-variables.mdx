...
| **AI Content Controls** ||||
| `AI_SENSITIVE_CONTENT_POLICY_DEFAULT` | No | Default policy for handling narrow sensitive-content matches in LLM requests (`ALLOW`, `REDACT`, or `BLOCK`) | `ALLOW` |
| `AI_SENSITIVE_CONTENT_POLICY_LOCKED` | No | Set to `true` to enforce the default policy for all accounts and disable account-level edits | `false` |
