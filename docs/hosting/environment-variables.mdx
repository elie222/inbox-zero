...
| `MICROSOFT_OAUTH_ENABLED` | No | Enable Microsoft sign-in and provider registration when credentials are configured. Set to `false` to hide the Microsoft login option without removing credentials. | `true` |
| **Feature Flags** ||||
...
| `NEXT_PUBLIC_SSO_LOGIN_BUTTON_ENABLED` | No | Show the "Sign in with SSO" button on the login screen. Disable this when your deployment does not use organization SSO. | `false` |
