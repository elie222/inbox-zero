...
| `LOGIN_PROVIDERS` | No | Comma-separated allowlist of login providers to expose and allow, e.g. `google,microsoft,sso`. Valid tokens: `google`, `microsoft`, `apple`, `sso`. Unset shows Google, Microsoft when configured, and SSO when `SSO_LOGIN_ENABLED=true`; Apple is shown only when explicitly allowlisted. | Default providers |
| **Feature Flags** ||||
