...
| `LOGIN_PROVIDERS` | No | Comma-separated allowlist of login providers to expose, e.g. `google,microsoft,sso`. Valid tokens: `google`, `microsoft`, `apple`, `sso`. Enforced both in the UI and in the auth backend, so providers outside the list cannot be used. Unset shows every provider that is otherwise configured. | All configured providers |
| **Feature Flags** ||||
