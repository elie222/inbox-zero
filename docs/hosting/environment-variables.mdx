...
| `DEFAULT_LLM_PROVIDER` | Yes | Primary LLM provider (`anthropic`, `azure`, `google`, `openai`, `bedrock`, `openrouter`, `groq`, `aigateway`, `ollama`) | — |
...
| `OPENAI_COMPATIBLE_MODEL` | No | Model name to use (e.g. `llama-3.2-3b-instruct`) | — |
| `OPENAI_COMPATIBLE_API_KEY` | No | API key, if required by the server (many local servers do not need one) | — |
...
| **Other** ||||
| `CRON_SECRET` | No | Secret for cron job authentication. Required for Vercel cron auth (`Authorization: Bearer <CRON_SECRET>`). | — |
