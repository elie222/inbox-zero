...
| `DEFAULT_LLM_FALLBACKS` | No | Ordered fallback chain (`provider[:model],provider[:model]`) | — |
...
| `ECONOMY_LLM_FALLBACKS` | No | Fallback chain for economy model type | — |
...
| `CHAT_LLM_FALLBACKS` | No | Fallback chain for chat model type | — |
