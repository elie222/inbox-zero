...
7. **Configure API permissions:**
   ...
   - If `NEXT_PUBLIC_CONTACTS_ENABLED=true`, also add `People.Read` for contact suggestions while composing.
   - Existing accounts can continue using core email features without `People.Read`. The composer offers an optional reconnect action if they want contact suggestions.
