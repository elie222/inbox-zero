...
3. **Update `.env` file:**
   ...
   - Optional for local OAuth emulation: set `GOOGLE_OAUTH_BASE_URL=http://localhost:4002`.
...
## Local OAuth emulation

For local development you can emulate the Google OAuth/OIDC flow with [`emulate`](https://emulate.dev/):
...
The emulator only covers OAuth/OIDC. Gmail, Calendar, Drive, Contacts, and Pub/Sub setup still require real Google services.
