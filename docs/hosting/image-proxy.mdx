...
For AWS and Cloudflare deployments, the proxy can also accept a comma-separated list of secrets if you need to validate requests from multiple clients.
Each client that signs proxy URLs should still use a single secret.
...
Because the Next.js app route signs and validates in the same process, keep `IMAGE_PROXY_SIGNING_SECRET` to a single value in this mode.
If you need the proxy to accept multiple signer secrets, use a separate proxy deployment.
