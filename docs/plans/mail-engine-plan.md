...
Build a fast, consistent web/Electron mail client around a shared SQLite engine while preserving the always-on server assistant. This is the single design and implementation reference. Use the [goal prompt](./mail-engine-goal-prompt.md) to authorize execution and the [task ledger](./mail-engine-tasks.md) to track verified progress and resume work.
...
For each package retain: exact command, provider/runtime, seed, pass/fail/skip counts, artifact locations, and unresolved limitations in the existing [task ledger](./mail-engine-tasks.md). Keep that ledger as the single resume checkpoint rather than creating a second status file. Record proposed decisions separately from verified behavior. Do not mark blocked/skipped/platform-unavailable cells passed.
...
Use the [goal prompt](./mail-engine-goal-prompt.md) and [task ledger](./mail-engine-tasks.md) for handoff. Ensure the plan, goal prompt, and task ledger exist in the implementation checkout; initially untracked files are not automatically copied into a new worktree.
