...
- Last implementation commit: `0ad57446b`
...
- Next action: remaining desktop UI matrix cells, live multi-account fencing, and watch CI on the exact head after this ledger commit.
...
| Account/owner/session isolation | Partial: follower tab + owner reload (E14); worker in-flight fence + wrong-account follower (E28) | Partial: follower tab + owner reload + reconnect (E21) | Partial: Electron process owns SQLite (E17); local MailApp `file:` boot (E22); linux-unpacked `INBOX_ZERO_LOCAL_MAIL=1` (E23) | Not run | Worker account fence + Web Lock owner + follower-tab channel + forked utility-child |
...
- Limitations: still a single primary inbox stream; expired-delta 410 rebuild and per-folder discovery UI remain unproven.
