...
1. Open [Settings](https://www.getinboxzero.com/settings) and scroll to **Developer → API Access**.
2. Turn on **MCP**.
...
Self-hosted deployments use `https://your-inbox-zero-host/mcp` after setting `MCP_SERVER_ENABLED=true` and `NEXT_PUBLIC_EXTERNAL_API_ENABLED=true`. See the [MCP API reference](/api-reference/mcp).
...
A Cursor plugin that bundles this MCP URL plus a skill is planned for later marketplace listing. The live MCP server is the prerequisite; you can connect with `mcp.json` today.
