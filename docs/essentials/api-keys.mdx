...
Create a key in [OrcaRouter](https://www.orcarouter.ai). Use a provider-qualified model identifier such as `anthropic/claude-sonnet-4.6`, or `orcarouter/auto` to let OrcaRouter route each request to the best model automatically.
