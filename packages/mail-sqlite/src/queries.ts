  ...
  switch (predicate.kind) {
    ...
    case "membership":
      if (predicate.membership === "folder") {
        return { sql: `${alias}.folder_id = ?`, bindings: [predicate.id] };
      }
      if (predicate.membership === "label") {
        return {
          sql: `EXISTS (SELECT 1 FROM json_each(${alias}.label_ids_json) WHERE value = ?)`,
          bindings: [predicate.id],
        };
      }
      return {
        sql: `EXISTS (SELECT 1 FROM json_each(${alias}.category_ids_json) WHERE value = ?)`,
        bindings: [predicate.id],
      };
    case "address": {
      const column =
