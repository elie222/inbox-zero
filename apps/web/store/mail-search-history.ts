...
export function rememberRecentSearch(emailAccountId: string, query: string) {
  if (!persistRecentSearches) return;
...
const SEARCH_HISTORY_STORAGE_PREFIX = "mail-search-history:";
let persistRecentSearches = true;

export function clearRecentSearchHistory() {
  persistRecentSearches = false;
  if (typeof window === "undefined") return;
  try {
    removePrefixedStorageKeys(
      window.localStorage,
      SEARCH_HISTORY_STORAGE_PREFIX,
    );
  } catch {
    // Private mode; in-memory search still works this session.
  }
}

export function clearRecentSearchHistoryForAccount(emailAccountId: string) {
  if (typeof window === "undefined") return;
  try {
...
function storageKey(emailAccountId: string) {
  return `${SEARCH_HISTORY_STORAGE_PREFIX}${emailAccountId}`;
}
