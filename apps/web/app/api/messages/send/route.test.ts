...
describe("POST /api/messages/send", () => {
  ...
  it("keeps accepting the legacy direct-send payload", async () => {
    ...
    expect(createEmailProvider).toHaveBeenCalledOnce();
    expect(executeDurableEmailSend).not.toHaveBeenCalled();
  });

  it("scopes provider lookup to the authenticated account owner", async () => {
    emailProvider.sendEmailWithHtml.mockResolvedValue({
      messageId: "message-1",
      threadId: "thread-1",
    });
  ...
  it("rejects attachment metadata over the direct mobile limit before provider work", async () => {
    const size =
      Math.floor(DURABLE_MULTIPART_EMAIL_SEND_LIMITS.maxAttachmentBytes / 2) +
      1;
    const input = durableInput([
      attachmentMetadata({ filename: "first.bin", size }),
      attachmentMetadata({ id: "attachment-2", filename: "second.bin", size }),
  ...
  it("rejects actual attachment bytes over the direct mobile limit before provider work", async () => {
    const size = DURABLE_MULTIPART_EMAIL_SEND_LIMITS.maxAttachmentBytes + 1;
    const input = durableInput([attachmentMetadata({ size: 1 })]);

    await expect(
      postMultipart(input, [
        new File([new Uint8Array(size)], "notes.txt", {
          type: "text/plain",
        }),
      ]),
    ).rejects.toThrow(DURABLE_MULTIPART_ATTACHMENT_LIMIT_MESSAGE);
    expect(findEmailAccount).not.toHaveBeenCalled();
    expect(createEmailProvider).not.toHaveBeenCalled();
    expect(executeDurableEmailSend).not.toHaveBeenCalled();
  });
