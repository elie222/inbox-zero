...
const logger = createScopedLogger("api/webhooks/deep-clean/archive");

/**
 * Webhook handler called by QStash to process archive operations.
 * This is NOT a user-facing mutation - users call archiveCategoryAction which queues this job.
 */
export async function POST(request: Request) {
  try {
    ...
    if (allThreadIds.length === 0) {
      logger.info("No threads found to archive", {
        emailAccountId,
        operationId,
      });
      return NextResponse.json({
        ok: true,
        message: "No threads found to archive",
        operationId,
      });
    }
      ...
      await Promise.all(
        batch.map(async (threadId) => {
          try {
            ...
            errorCount++;
            logger.warn("Failed to archive thread", {
              threadId,
              error,
            });
          }
    ...
    return NextResponse.json({
      ok: true,
      message: `Archived ${successCount} emails${errorCount > 0 ? ` (${errorCount} failed)` : ""}`,
      operationId,
      successCount,
      errorCount,
    });
  } catch (error) {
    logger.error("Archive operation error", { error });
