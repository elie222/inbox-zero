  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {ruleHistory.length === 0 ? (
        <p className="text-muted-foreground mt-4">No history found for this rule.</p>
      ) : (
            ...
            <Card key={history.id}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="outline">
                      {triggerTypeLabels[history.triggerType] || history.triggerType}
                    </Badge>
                    <span className="text-sm text-muted-foreground">
                      {formatDistanceToNow(history.createdAt, { addSuffix: true })}
                    </span>
                  </div>
                </div>
                {history.promptText && (
              ...
              <CardContent>
                <div className="space-y-3">
                  <div>
                    <h4 className="font-semibold mb-1">Rule Details</h4>
                  ...
                  {(history.from || history.to || history.subject || history.body) && (
                    <div>
                      <h4 className="font-semibold mb-1">Static Conditions</h4>
                      <dl className="grid grid-cols-1 gap-1 text-sm">
                        {history.from && (
                          <div className="flex gap-2">
                  ...
                  {history.categoryFilterType && (
                    <div>
                      <h4 className="font-semibold mb-1">Category Filters</h4>
                      <p className="text-sm">
                        Type: {history.categoryFilterType}
                        {history.categoryFilters && (
                          <span className="ml-2">
                            ({(history.categoryFilters as any[]).length} categories)
                          </span>
                        )}
                      </p>
                    </div>
                  ...
                  {history.actions && (
                    <div>
                      ...
                      <div className="space-y-1">
                        {(history.actions as any[]).map((action: any, index: number) => (
                          <div key={index} className="text-sm">
                            <Badge variant="secondary" className="mr-2">
                              {action.type}
                            </Badge>
                            {action.label && <span>Label: {action.label}</span>}
                            {action.subject && <span>Subject: {action.subject}</span>}
                            {action.content && <span>Content: {action.content.substring(0, 50)}...</span>}
