...
const availableOptions = [
  ...
  { name: "Mark as read", icon: MailCheck },
  { name: "Forward", icon: Forward },
  { name: "Draft replies", icon: PenLine },
...
export function CustomRulesModalIllustration() {
  ...
  const getCursorPosition = () => {
    switch (phase) {
      ...
      case "moving-to-button":
        return { x: -170, y: 128, opacity: 1 };
      case "clicking":
        return { x: -170, y: 131, opacity: 1 };
      case "loading":
      case "showing-rules":
      case "pause":
        return { x: -170, y: 128, opacity: 0 };
      default:
        return { x: 100, y: -60, opacity: 0 };
    }
  };
  ...
  return (
    <div className="relative flex h-[280px] w-[520px] items-start justify-center gap-4">
      <motion.div
        ...
        {availableOptions.map((option, index) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[11px] text-gray-700">{option.name}</span>
            </motion.div>
          );
        })}
      </motion.div>
