...
function buildPricingMap(payload: OpenRouterModelsResponse) {
  ...
  for (const model of payload.data) {
    ...
    if (!pricing) continue;
    pricingByModelId[model.id] = pricing;
  }
  ...
  for (const supportedModelId of supportedModelIds) {
    if (pricingByModelId[supportedModelId]) continue;

    const candidateModelIds =
      buildOpenRouterModelIdCandidates(supportedModelId);
    const matchedPricing = candidateModelIds
      .map((candidateModelId) => pricingByModelId[candidateModelId])
      .find(Boolean);
    ...
    pricingByModelId[supportedModelId] = matchedPricing;
  }
