...
describe("findMatchingRules - Integration Tests", () => {
  ...
  it("shortcuts CATEGORY_PROMOTIONS-only message to Marketing rule with PRESET match reason and no AI call", async () => {
    const marketingRule = getRule({
      id: "marketing-rule",
      systemType: SystemType.MARKETING,
      instructions: null,
    });
    // Static-only rule that won't match this sender — proves the shortcut
    // works alongside non-AI rules without triggering aiChooseRule.
    const unrelatedStaticRule = getRule({
      id: "static-rule",
      from: "someone-else@example.com",
    });

    const rules = [marketingRule, unrelatedStaticRule];
    const message = getMessage({
  ...
  it("does not shortcut multi-category messages (PROMOTIONS + UPDATES)", async () => {
    const marketingRule = getRule({
      id: "marketing-rule",
      systemType: SystemType.MARKETING,
    });
    const aiRule = getRule({
      id: "ai-rule",
      instructions: "Catch-all AI rule",
    });
    ...
    const rules = [marketingRule, aiRule];
    const message = getMessage({
    ...
    // No PRESET/MARKETING shortcut should have fired.
    const marketingPreset = result.matches.find(
      (m) =>
        m.rule.id === "marketing-rule" &&
        m.matchReasons?.some(
          (r) =>
            r.type === ConditionType.PRESET &&
            r.systemType === SystemType.MARKETING,
        ),
    );
    expect(marketingPreset).toBeUndefined();
    expect(aiChooseRule).toHaveBeenCalled();
  });

  it("learned-pattern match wins over PROMOTIONS shortcut on a different rule", async () => {
    const marketingRule = getRule({
      id: "marketing-rule",
      systemType: SystemType.MARKETING,
    });
    const newslettersRule = getRule({
      id: "newsletters-rule",
      groupId: "newsletters-group",
    });

    prisma.group.findMany.mockResolvedValue([
      getGroup({
        id: "newsletters-group",
        items: [
          getGroupItem({
            type: GroupItemType.FROM,
        ...
        rule: newslettersRule,
      }),
    ]);

    const rules = [marketingRule, newslettersRule];
    const message = getMessage({
  ...
  it("falls through to LLM when no Marketing rule exists", async () => {
    const aiRule = getRule({
      id: "ai-rule",
      instructions: "Decide what to do",
    });

    vi.mocked(aiChooseRule).mockResolvedValue({
      rules: [{ rule: aiRule as any }],
      reason: "AI handled it",
    });
