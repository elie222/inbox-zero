...
function getOutlookLabels(
  ...
  // Add category labels - map names to IDs if category map is available
...
export async function getMessage(
  ...
  return convertMessage(message, folderIds, logger, categoryMap);
}
...
export async function getMessages(
  ...
  // Get folder IDs and category map to properly map labels
  const [folderIds, categoryMap] = await Promise.all([
    getFolderIds(client, logger),
    getCategoryMap(client, logger),
  ]);
...
export function convertMessage(
  message: Message,
  folderIds: Record<string, string> = {},
  loggerOrCategoryMap?: Logger | Map<string, string>,
  categoryMap?: Map<string, string>,
): ParsedMessage {
  ...
  // Handle backwards compatibility: 3rd arg can be logger or categoryMap
  let logger: Logger | undefined;
  let resolvedCategoryMap: Map<string, string> | undefined;
  if (loggerOrCategoryMap instanceof Map) {
    resolvedCategoryMap = loggerOrCategoryMap;
  } else {
    logger = loggerOrCategoryMap;
    resolvedCategoryMap = categoryMap;
  }

  const labelIds = getOutlookLabels(message, folderIds, resolvedCategoryMap);
