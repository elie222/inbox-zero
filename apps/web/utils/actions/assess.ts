...
async function performWritingStyleAnalysis({
  emailAccountId,
  provider,
  logger,
}: {
  ...
  // biome-ignore lint/suspicious/noExplicitAny: type is complex, see actionClient
  logger: any;
}) {
  const emailAccount = await prisma.emailAccount.findUnique({
    where: { id: emailAccountId },
...
export const analyzeWritingStyleAction = actionClient
  ...
  .action(async ({ ctx: { emailAccountId, provider, logger } }) => {
    ...
    return performWritingStyleAnalysis({
      emailAccountId,
