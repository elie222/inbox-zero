...
describe("createGenerateText fallback chain", () => {
  ...
  it("promotes legacy openrouter extraBody trace into top-level trace", async () => {
    const model = { id: "openrouter-model" };
    const modelOptions: SelectModel = {
      provider: "openrouter",
      modelName: "openrouter-primary",
        ...
        openrouter: {
          extraBody: {
            trace: {
              ...
              generation_name: "legacy-generation",
              email_account_id: "legacy-email-account-id",
            },
          },
        },
      },
    ...
    expect(providerOptions.openrouter.extraBody).toBeUndefined();
  });
});
