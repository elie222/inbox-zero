...
vi.mock("@/utils/prisma", () => ({
  default: {
    ...
    groupItem: { deleteMany: vi.fn().mockResolvedValue({ count: 0 }) },
  },
}));
...
describe("learn-from-label", () => {
  ...
  describe("learnSenderFromLabel", () => {
    ...
    it("never learns from an app-managed label, even into an existing rule", async () => {
      getLabelById.mockResolvedValue({
        id: "Label_1",
        name: "Inbox Zero/Unsubscribed",
      });

      await learnSenderFromLabel({ ...baseArgs, ruleId: "rule-unsub" });
      await learnSenderFromLabel({ ...baseArgs, ruleId: null });
  ...
  describe("keepSenderInInbox", () => {
    it("clears the unsubscribe, drops training and excludes the sender from every enabled rule", async () => {
