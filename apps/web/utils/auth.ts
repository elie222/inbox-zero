...
import { getUserProfile, getUserPhoto } from "@/utils/outlook/client";
...
const PROVIDER_CONFIG = {
  ...
  "microsoft-entra-id": {
    ...
    getProfileData: async (accessToken: string) => {
      ...
      try {
        const profileResponse = await getUserProfile(client);

        // Get photo separately as it requires a different endpoint
        let photoUrl = null;
        try {
          const photo = await getUserPhoto(client);
          if (photo) {
...
export const getAuthOptions: (options?: {
  ...
  providers: [
    ...
    MicrosoftProvider({
      ...
      profile(profile) {
        logger.info("[MicrosoftProvider] Raw profile data:", { profile });
        ...
        logger.info("[MicrosoftProvider] Extracted profile data:", {
          profileData,
        });
      ...
      async userinfo(tokens: { access_token: string }) {
        try {
          const client = getOutlookContactsClient({
          ...
          logger.error(
            "[MicrosoftProvider] Error fetching user info from Graph API",
            {
              error,
              message: "Will use profile data from token instead",
            },
          );
          // Return null to make the provider fall back to using the profile data
          return null;
        }
      },
    }),
