...
export const Button = (props: ButtonProps) => {
  const { color, size = 'md', roundedSize = 'md', full, loading, ...rest } = props;
      ...
      className={clsx(
        ...
        }[roundedSize],
        {
          ...
          'border-x-0 bg-gradient-to-r from-green-400 via-green-500 to-green-600 text-white':
            color === 'green',
          'bg-green-200 text-gray-900 hover:bg-green-300': color === 'light-green',
...
const BasicLink = (props: {
  link: { href: string; target?: React.HTMLAttributeAnchorTarget; rel?: string | undefined };
  children: React.ReactNode;
  type?: string;
}) => {
  const {
