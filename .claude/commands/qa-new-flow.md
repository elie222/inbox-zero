...
- Optional: `--category=settings --estimated-duration=60s --requires=authenticated_session,gmail_account --conflicts-with=other-flow-id,another-flow-id --preconditions="Signed in" --cleanup="Remove test rule"`
...
5. If optional fields are provided (`category`, `estimated_duration`, `requires`, `conflicts_with`, `timeout_minutes`, `tags`, `preconditions`, `cleanup`), include them.
