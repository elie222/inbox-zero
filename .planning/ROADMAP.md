...
- [x] **Phase 4: Daily Digest** - 9:00 AM ET digest with Sonnet narrative + per-item Urgent/Uncertain + Sonnet-clustered auto-filed roll-ups *(completed 2026-05-17 — 9am ET auto-fire verified organically; "Wrong label?" feedback button deferred to Phase 6 since classification has been accurate enough that user hasn't needed it)*
- [x] **Phase 5: Rules Management UI** - Simple page at inbox.tdfurn.com/rules for explicit classification instructions *(completed 2026-05-17 — satisfied by upstream features unknown at spec time: `/rules` already redirects to the upstream RuleForm UI which covers plain-language rules, sender-only rules, edit/delete, and runtime serialization via `getUserRulesPrompt`. Empirically validated by 13 days of accurate classification post-Phase 4 deploy with 8 active rules. No new code written.)*
- [x] **Phase 6: Feedback System** - In-email signals, narrative form, and Gmail behavioral feedback feeding classification *(completed 2026-05-17 — closed via fork-revert. Real-world classification accuracy made the fork's `/feedback` page, `/api/user/rule-feedback` endpoint, and digest `feedbackUrl` wiring unnecessary; all three were deleted to ease future upstream merges. FEEDBACK-03/04/05 satisfied by upstream `/ai/chat` UI + existing Gmail history label/delete detection. FEEDBACK-06 deferred — revisit only if accuracy degrades.)*
- [x] **Phase 7: Backlog Triage** - AI-assisted processing of 100k+ existing emails with batch proposals and human approval *(completed 2026-05-17 — satisfied by manual triage. User worked the backlog down using Gmail's native tools: size-filtering, Promotions purge, mailing-list cleanup, top-sender filtering, unsubscribe+block. The remaining long-tail mail no longer justifies building classify-and-batch infrastructure. Live pipeline from Phases 1-6 handles new mail going forward.)*
...
**Goal**: Every morning at 9:00 AM ET, Rebekah receives a single digest email with a Sonnet-written narrative on top, per-item Urgent/Uncertain cards, and Sonnet-clustered auto-filed roll-ups (Receipts, Newsletters, Marketing including "Deals — {topic}" sub-clusters, Notifications). DIGEST-07 is descoped to Phase 6.
**Depends on**: Phase 3
**Requirements**: DIGEST-01, DIGEST-02, DIGEST-03, DIGEST-04, DIGEST-05, DIGEST-06 (DIGEST-07 deferred to Phase 6)
**Success Criteria** (what must be TRUE):
  1. A digest email arrives in rebekah@trueocean.com at 9:00 AM ET (since-last-send window, no upper cap)
  2. Urgent and Uncertain items render as per-item cards with subject, sender, and Sonnet-written ≤25-word summary
  3. Auto-filed sections (Receipts, Newsletters, Marketing, Notifications) render Sonnet-clustered roll-up rows (one row per cluster, label + ≤30-word summary)
  4. Marketing rows for promotional items use the "Deals — {topic}" label prefix (D-18; no separate Deals section)
  5. Each Uncertain item has a single "Review in app →" deep-link to https://inbox.tdfurn.com/uncertain/{itemId}
  6. The Sonnet call is one batched generateObject per digest; output drives all narrative + per-item summaries + cluster labels
  7. Idempotent: a second cron fire on the same ET date short-circuits via DigestSend.findUnique without sending a duplicate email
  8. Hard tone guardrail in the system prompt drops humor entirely on grief / illness / legal / distress / medical content (D-04)
**Plans**: 6 plans in 4 waves

Plans:
- [ ] 04-01-PLAN.md — REQUIREMENTS.md spec edits + Wave 0 test scaffolds + DigestItem-gap instrumentation
- [ ] 04-02-PLAN.md — Prisma DigestSend model + [BLOCKING] migration + generate
- [ ] 04-03-PLAN.md — Marketing-rule DIGEST action seed + 218-row backfill SQL + production apply checkpoint
- [ ] 04-04-PLAN.md — Sonnet batched-content module (digest-schema, digest-prompt with HARD GUARDRAIL, generate-digest-content)
- [ ] 04-05-PLAN.md — Cron route + run-daily-digest orchestrator + idempotency + today-et + sendDigestV2Email wrapper
- [ ] 04-06-PLAN.md — systemd timer + service unit + install/smoke-test checkpoint
**UI hint**: yes
