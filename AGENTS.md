...
- Use `z.strictObject()` only when unknown keys must cause validation to fail. Use `z.object()` when stripping unknown keys is acceptable; do not choose between them for LLM compatibility because both are exposed to models as closed objects by the current schema conversion.
